...
export default function PurchaseWithAIDialog({ open, onOpenChange, onSuccess }: PurchaseWithAIDialogProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // -----------------------------------------------------------------
      // ETAPA 1: VERIFICAR SALDO (Nova chamada)
      // -----------------------------------------------------------------
      ...
      if (balanceError) {
        ...
        try {
          // O Supabase envia o erro 400 como um JSON stringuificado
          const errorBody = JSON.parse(balanceError.message);
          if (errorBody.error === "insufficient_balance") {
            errorMessage = errorBody.message; // "Saldo insuficiente! Você tem $X..."
          } else {
        ...
        setShowProgress(false); // Esconde o progresso
      ...
      // -----------------------------------------------------------------
      // ETAPA 2: INICIAR A COMPRA (Se o saldo estiver OK)
      // -----------------------------------------------------------------
      console.log("Iniciando a compra...");
      ...
      if (purchaseError) {
        // Erro ao *iniciar* a compra (ex: n8n webhook fora do ar)
      ...
      // -----------------------------------------------------------------
      // ETAPA 3: INSCREVER NO REALTIME (Lógica original mantida)
      // -----------------------------------------------------------------
      resetTimeout();
      ...
      // Pega erros de qualquer uma das etapas (check-balance ou purchase-hub)
